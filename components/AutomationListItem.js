...
const AutomationItem = ({type, onPress}) => {
  const [isEnabled, setIsEnabled] = useState(false);
  const toggleSwitch = () => setIsEnabled(previousState => !previousState);

  return (
    <TouchableOpacity className="mx-auto w-[90%] h-20 flex-1 flex-row justify-between bg-sky-500 rounded-lg drop-shadow-lg" onPress={() => onPress(type)}>
